...
def main():
    ...
    # --- Updated cloning logic ---
    def plexoscloud_exists(base_path):
        # Ensure that we check directly in the output path for the .plexoscloud folder
        if os.path.isdir(os.path.join(base_path, ".plexoscloud")):
            return True
        return False

    if not plexoscloud_exists(output_path):
        APILogger.info(f"No .plexoscloud folder found in {output_path}. Attempting to clone study...")
        try:
            # Use the correct clone_study method to clone the study
            command_responses = pxc.study.clone_study(study_id=study_id, output_directory_path=output_path, print_message=True)
            
            # Check the response for successful cloning
            last_command_response: CommandResponse[Contracts_CloneStudyResponse] = pxc.solution.get_final_response(command_responses)
            
            if last_command_response is not None and last_command_response.Status == "Success":
                APILogger.info(f"Study successfully cloned into {output_path}.")
            else:
                APILogger.error(f"Failed to clone study: {last_command_response.Status}")
                return

        except Exception as e:
            APILogger.error(f"Error while attempting to clone study: {str(e)}")
            return
    else:
        APILogger.info(".plexoscloud folder exists in the output path. Skipping clone.")
    # --- End cloning logic ---
